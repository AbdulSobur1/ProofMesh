...
const buildMobilePrimaryLinks = (currentUser: SessionUser | null) => {
  const links = buildSidebarLinks(currentUser)
  const allowed = new Set(['/dashboard', '/discover', '/submit', '/messages'])

  return [
    ...links.filter((link) => allowed.has(link.href)),
    links.find((link) => link.label === 'Profile'),
...
export function Sidebar() {
  ...
  const links = buildSidebarLinks(currentUser)
  const mobileLinks = buildMobilePrimaryLinks(currentUser)
  ...
  return (
    <>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border/60 bg-background/95 px-2 pb-[calc(env(safe-area-inset-bottom)+0.4rem)] pt-2 backdrop-blur md:hidden">
        <div className="mx-auto flex max-w-lg items-center justify-between gap-1">
          {mobileLinks.map((link) => {
            ...
            return (
              ...
              >
                <Icon className="size-4" />
                <span className="truncate">{link.label.replace('Discover Talent', 'Discover').replace('Submit Proof', 'Submit')}</span>
              </Link>
            )
          })}
        </div>
